...
export default async function OnboardingPage({
  ...
  let initial: { name: string; nationality: string; status: string; bio: string; profileImage: string; socialLinks: Record<string, string> } | undefined;

  try {
    const profile = await backendFetch<ApiProfile>('/users/me');
    isReturning = profile.onboarding_complete;
    if (isReturning) {
      if (!isEditMode) redirect('/meetups');
      initial = {
        name: profile.name ?? '',
